        ...
        <div className='mt-2'>
            <div className='py-2 px-4 border border-blue-600/30 rounded flex justify-between shadow-sm'>
                ...
                <form className='flex items-center'>
                    ...
                    <label style={{ WebkitAnimationIterationCount: 0.5, animationIterationCount: 0.5 }} className={isEdit ? "hidden" : props.data?.is_checked ? "duration-75 ease-linear animate-bounce line-through" : ""} htmlFor={"checkbox_" + props.data?.id}>{props.data?.name}</label>
                    <div className={isEdit ? "flex flex-nowrap" : "hidden"}>
                        <input onChange={(e) => setInputEdit({ ...props.data, name: e.target.value })} className="flex-1 border-b border-black/20 shadow-blue-50 focus:outline-none" value={inputEdit.name} />
                        <button onClick={() => editTask()} className="flex-none bg-black text-white text-xs px-1 py-0 rounded">OK</button>
                    </div>
                </form>

                <div className={isEdit ? "hidden" : "flex flex-nowrap"}>
                    {/* edit button */}
                    <button className='mx-2 bg-blue-100 px-1 rounded-full active:bg-white' onClick={() => setIsEdit(!isEdit)}>
