...
const TaskCard = (props) => {
    ...
    const markChecked = async () => {
        try {
            await axios.put("http://localhost:3000/todos/" + props.data.id, {
                "name": "Task 1",
                "is_checked": !props.data.is_checked,
            });
            props.setIsDataUpdated(false);
        } catch (error) {
            console.log(error);
    ...
    return (
        <div className='w-64 mt-2'>
            <div className='py-2 px-4 border-2 border-black flex gap-2 justify-between'>
                ...
                <form className=''>
                    <input onChange={() => markChecked()} className='me-2 border-2 border-black' type='checkbox' id="task-1" name="task-1" checked={props.data.is_checked && 'checked'} />
